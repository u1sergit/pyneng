...
for intf, vlan in fast_int['trunk'].items():
    ...
    for command in trunk_template:
        if command.endswith('encapsulation dot1q'):
            if vlan[0].endswith('add'):
                for vl in vlan[1:]:
                    print('add {} {}'.format(command, vl))
            elif vlan[0].endswith('only'):
                for vl in vlan[1:]:
                    print('only {} {}'.format(command, vl))
            elif vlan[0].endswith('del'):
                for vl in vlan[1:]:
                    print('del {} {}'.format(command, vl))
